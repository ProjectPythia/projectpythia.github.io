...
def _generate_menu(all_items):
    ...
    menu_html += '<div><button class="btn btn-primary btn-sm mx-1" onclick="clearCbs()">Clear all Filters</button></div>\n'
...
def build_from_items(items, filename, title='Gallery', subtitle=None, menu_html='', max_descr_len=300):
    ...
    for item in items:
        ...
        tags = [f'<button class="btn btn-primary btn-sm">{tag}</button>' for tag in tag_list_f]
        tags = '\n'.join(tags)
